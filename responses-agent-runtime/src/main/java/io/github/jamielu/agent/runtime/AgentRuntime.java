...
public final class AgentRuntime {
    ...
    public AgentRuntime(AgentModelPort model, ToolRegistry tools, int maxSteps) {
        this(singletonFactory(model), tools, maxSteps);
    }
    ...
    /**
     * 创建每次 {@link #run(String)} 都获得独立模型会话的 Runtime。
     *
     * <p>有状态 adapter（例如使用 {@code previous_response_id} 的 Responses
     * adapter）应通过该工厂接入，以避免不同运行共享协议状态。</p>
     *
     * @param modelFactory 每次运行调用一次的模型端口工厂
     * @param tools 工具注册表
     * @param maxSteps 最大步骤数，必须为正数
     * @return 可安全重复运行且模型状态按运行隔离的 Runtime
     */
    public static AgentRuntime withModelFactory(
            Supplier<? extends AgentModelPort> modelFactory,
            ToolRegistry tools,
            int maxSteps) {
        return new AgentRuntime(modelFactory, tools, maxSteps);
    }
